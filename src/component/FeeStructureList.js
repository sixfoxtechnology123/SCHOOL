    ...
    try {
      ...
      const feesWithAmount = await Promise.all(
        res.data.map(async (fee) => {
          // Fetch amount dynamically for each fee
          try {
            const amtRes = await axios.get(
              `http://localhost:5000/api/fees/get-amount?classId=${fee.classId}&feeHeadId=${fee.feeHeadId}`
            );
            return { ...fee, amount: amtRes.data.amount || 0 };
          } catch (err) {
            console.error("Failed to fetch amount for fee:", fee, err);
            return { ...fee, amount: fee.amount || 0 };
          }
        })
      );
        ...
        <table className="w-full table-auto border border-green-500">
          <thead className="bg-green-100 text-sm">
            <tr>
              ...
              <th className="border border-green-500 px-2 py-1">Transport</th>
